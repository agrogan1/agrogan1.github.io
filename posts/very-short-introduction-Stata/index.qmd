...
| Describe the data               | `describe x y z` [^describe]  |
| Codebook of variables           | `codebook x y z`              | 
| Descriptive statistics          | `summarize x y`               |
| Frequencies                     | `tabulate x`                  |
| Cross-Tabulation.               | `tabulate x y` [^rowcol]      |
| Cronbach's alpha                | `alpha x y z` [^item]         |
...
| Multilevel Model                | `mixed y x z || group: x` [^melogit] |
...
| Scatterplot and Linear Fit      | `twoway (scatter y x) (lfit y x)` |
...
[^item]: After the `,` the `item` option will estimate the increase in *alpha* if the particular item were removed. 
...
[^melogit]: An equivalent multilevel *logistic* regression model would be `melogit y x z || group: x, or`.
