...
The data used in this example are derived from the R package *Functions and Datasets for "Forest Analytics with R"* [@FAwR].
...
graph export myhistogram.png, width(2000) replace
...
graph export mybargraph.png, width(2000) replace
...
graph export mybargraph_asyvars.png, width(2000) replace
...
graph export myscatter.png, width(2000) replace
...
graph export mybargraph2.png, width(2000) replace
...
graph export mybargraph3.png, width(2000) replace
...
I have written a [Stata Michigan graph scheme](https://agrogan1.github.io/Stata/) that can be installed. Asjad Naqvi has written [an excellent and comprehensive set of Stata graph schemes](https://github.com/asjadnaqvi/Stata-schemes), including the `neon` scheme used in some examples in this blog post.
...
graph export myscatterM.png, width(2000) replace
...
twoway scatter height age_base, title("Tree Height by Age") scheme(neon) msymbol(o)

graph export myscatterNEON.png, width(2000) replace

```

![Scatterplot Of Tree Height By Age With neon Graph Scheme](myscatterNEON.png){width=50%}
...
graph export myscatterS.png, width(2000) replace
...
graph export mybarM.png, width(2000) replace
...
graph bar height, over(location) asyvars title("Tree Height by Location") scheme(neon)
	
graph export mybarNEON.png, width(2000) replace

```

![Bar Graph Of Mean Tree Height By Location With neon Graph Scheme](mybarNEON.png){width=50%}
```{stata}
#| output: false
#| echo: true
...
graph export mybarS.png, width(2000) replace
