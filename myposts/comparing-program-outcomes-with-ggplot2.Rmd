...
> In HTML versions of this document, you can see the code by clicking on the `code` buttons on the right hand side of the document.
...
myplot1 + geom_boxplot(fill="white") + # boxplot geometry
  labs(title = "Boxplot")
...
myplot1 + geom_violin() + # violinplot geometry
...
myplot1 + geom_point() + # point geometry
  labs(title = "Points")
...
myplot1 + geom_jitter() + # jittered point geometry
...
myplot1 + geom_beeswarm() + # beeswarm geometry
...
myplot2 + geom_histogram() + # histogram geometry
...
myplot2 + geom_density() + # density geometry
  labs(title = "Density")
