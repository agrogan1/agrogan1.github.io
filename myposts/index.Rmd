...
# get info about my PDF files

myPDFfiles = file.info(list.files(pattern="*pdf"))

save(myPDFfiles, file = "myPDFfiles.RData")
...
rownames_to_column(var = "filename") %>% # rownames to column
  filter(filename != "blogtest.html") %>% # filter out these file names
  ...
  datatable(rownames = FALSE, # no row names
            ...
            options = list(order = list(list(0, 
                                             'desc')))) %>% 
...
myPDFfiles %>% 
rownames_to_column(var = "filename") %>% # rownames to column
  filter(filename != "Rplots.pdf") %>% # filter out these file names
  mutate(modified = mtime) %>% # copy mtime to modified
  mutate(url = paste0("<a href='", # paste together URL
                      filename,
                      "'>",
