...
# If you are following the `ggplot` code, 
# you will note that `ggplot` rapidly 
# becomes very verbose. 
# 
# The *essential detail* to focus on is 
# the *aesthetic*, `aes`, which tells ggplot 
# what *dimensions* of the data are being 
# mapped on to what specific elements of the graph.  
# 
# The data we are using only has 1 *dimension*: 
# of what group is each individual a member?
# 
# Much of the verbosity of `ggplot` comes 
# from telling `ggplot` about the *labels*, 
# *fill colors*, and *theme*.
...
I'm going to rely on Wilkinson's idea of a *Grammar of Graphics*. Wilkinson talks about such a grammar being characterized by *has a* relationships which I have indicated here with `->` symbols. 
